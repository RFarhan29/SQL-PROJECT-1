...
SELECT 
    pizza_types.category, SUM(orders_details.quantity) AS ctg
FROM
    pizza_types
        JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
SELECT 
    ROUND(AVG(qty), 0) as avg_pizzas_order_per_day
FROM
    (SELECT 
...
rank() over(partition by category order by revenue desc) as z
from 
...
join orders_details
on orders_details.pizza_id=pizzas.pizza_id
group by pizza_types.category, pizza_types.name) as Y) as x
where z <=3;
